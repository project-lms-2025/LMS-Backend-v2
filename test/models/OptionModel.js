...
class OptionModel {
  static async createOption({ option_id, question_id, option_text, image_url = null, is_correct }) {
    ...
    `;
    try {
      await query(queryStr, [option_id, question_id, option_text, image_url, is_correct]);
  ...
  static async getOptionById(option_id) {
    try {
      const [rows] = await query("SELECT * FROM options WHERE option_id = ?", [option_id]);
      return rows[0] || null;
    } catch (err) {
  ...
  static async getOptionsByQuestionId(question_id) {
    try {
      const [rows] = await query("SELECT * FROM options WHERE question_id = ?", [question_id]);
      return rows;
    } catch (err) {
  ...
  static async getCorrectOptionsByTestId(test_id) {
    try {
      const [rows] = await query("SELECT * FROM options WHERE test_id = ? AND is_correct = 1", [test_id]);
      return rows;
    } catch (err) {
  ...
  static async updateOption(option_id, updatedFields) {
    ...
    try {
      await query(`UPDATE options SET ${setClause} WHERE option_id = ?`, [...Object.values(updatedFields), option_id]);
  ...
  static async deleteOption(option_id) {
    try {
      await query("DELETE FROM options WHERE option_id = ?", [option_id]);
      return { success: true };
    } catch (err) {
