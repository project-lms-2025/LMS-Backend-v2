...
class ClassController {
  static async createClass(req, res) {
    try {
      const { course_id, class_title, class_date_time, recording_url } =
        req.body;
  ...
  static async getClassesByCourseId(req, res) {
    try {
      const classes = await ClassService.getClassesByCourseId(
  ...
  static async deleteClass(req, res) {
    try {
      const response = await ClassService.deleteClass(req.params.class_id);
      res
        .status(204)
